...
const seniorId = Number(urlParams.get("id"));
...
async function loadSenior() {
  try {
    const res = await fetch(
      `${API_BASE}/admin/profile_senior/getone?id=${seniorId}`,
    );
    const s = await res.json();
    document.getElementById("seniorName").textContent = `${s.nom} ${s.prenom}`;
    document.getElementById("btnAjouter").href =
      `admin_create_planning_senior.php?id_senior=${seniorId}`;
  } catch {}
}
...
async function loadPlannings() {
  try {
    ...
    plannings = all.filter((p) => p.id_senior === seniorId);
  } catch {}
}
...
async function loadInterventions() {
  try {
    ...
    const all = await res.json();
    interventions = all.filter((i) => i.id_senior === seniorId);
  } catch {}
}
...
async function loadEvenements() {
  try {
    const res = await fetch(
      `${API_BASE}/admin/inscription_evenement/get_by_user?id=${seniorId}`,
    );
...
function renderCalendar() {
  ...
  evenements.forEach((e) => {
    ...
    const top = (debutH - START_HOUR) * ROW_HEIGHT;
    html += `<div style="
      position: absolute; top: ${top}px; height: 48px;
      left: calc((100% - 60px) * ${jourIndex} / 7 + 60px + 2px);
      width: calc((100% - 60px) / 7 - 4px);
  ...
  plannings.forEach((p) => {
    ...
    html += `<div style="
      ...
      border-radius: 6px; padding: 4px 6px; font-size: 11px; overflow: hidden; z-index: 10;">
      <div style="font-weight: 500; color: #92400E;">🔔 Rappel</div>
      <div style="display: flex; gap: 4px; margin-top: 2px;">
        <a href="admin_edit_planning_senior.php?id=${p.id_agenda}" style="font-size: 10px; padding: 1px 5px; border-radius: 3px; background: white; color: #374151; border: 1px solid #d1d5db; text-decoration: none;">✎</a>
        <button type="button" data-id="${p.id_agenda}" class="btn-delete" style="font-size: 10px; padding: 1px 5px; border-radius: 3px; background: #FEE2E2; color: #DC2626; border: 1px solid #FCA5A5; cursor: pointer;">×</button>
      </div>
    </div>`;
  });
  ...
  html += `<div style="display: flex; gap: 16px; padding: 12px 16px; border-top: 1px solid #e5e7eb; font-size: 11px;">
    ...
    <span style="display: flex; align-items: center; gap: 4px;"><span style="width: 12px; height: 12px; background: #FEF3C7; border: 1px solid #FCD34D; border-radius: 3px; display: inline-block;"></span> Rappel</span>
  </div>`;
...
function initDeleteModal() {
  ...
  document
    ...
    .addEventListener("click", async () => {
      try {
        const res = await fetch(
          `${API_BASE}/admin/planning_senior/delete?id=${deleteId}`,
          { method: "DELETE" },
        );
        if (!res.ok) throw new Error();
        plannings = plannings.filter((p) => p.id_agenda !== Number(deleteId));
        renderCalendar();
        closeModal();
