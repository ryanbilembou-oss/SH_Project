...
async function loadCategorie() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/admin/service/categorie_service/getone?id=${categorieId}`,
    );
...
async function handleSubmit() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/admin/service/categorie_service/update?id=${categorieId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nom_categorie: nom_categorie }),
      },
    );
