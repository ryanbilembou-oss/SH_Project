...
async function loadLitiges() {
  ...
  try {
    ...
    if (!litiges.length) {
      tbody.innerHTML = `<tr><td colspan="5" class="py-20 text-center text-gray-400"><span class="text-sm italic">Aucun litige trouvé.</span></td></tr>`;
      return;
    }
    tbody.innerHTML = litiges
      .map((l) => {
        ...
        return `
        <tr id="litige-row-${l.id_litige}" class="border-b border-gray-100 hover:bg-blue-50/40 transition-colors">
          <td class="py-4 px-5 text-sm font-semibold text-gray-700">#${l.id_intervention}</td>
          <td class="py-4 px-5 text-sm text-gray-600 max-w-xs truncate">${esc(l.motif)}</td>
          <td class="py-4 px-5 text-center">
            <span class="text-xs font-bold px-2 py-1 rounded uppercase ${statutClass}">${esc(l.statut)}</span>
          </td>
          <td class="py-4 px-5 text-center text-sm text-gray-500">${date}</td>
          <td class="py-4 px-5">
            <div class="flex items-center justify-center gap-3">
              <a href="admin_edit_litige.php?id=${l.id_litige}" class="group flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 hover:bg-blue-600 transition-colors">
                <i class="fas fa-edit text-blue-600 group-hover:text-white text-sm"></i>
              </a>
              <button type="button" data-id="${l.id_litige}" class="btn-delete group flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 hover:bg-red-600 transition-colors">
  ...
  document
    ...
    .addEventListener("click", async () => {
      try {
        ...
        closeModal();
        showToast("Litige supprimé.", "success");
      } catch {
