...
async function loadType() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/admin/type_prestataire/getone?id=${typeId}`,
    );
    if (!res.ok) throw new Error();
    const t = await res.json();
    document.getElementById("nom_type").value = t.nom_type || "";
  } catch {
    showToast("Impossible de charger le type.", "error");
  }
}
...
async function handleSubmit() {
  ...
  try {
    const res = await fetch(`${API_BASE}/admin/type_prestataire/update`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id_type: Number(typeId), nom_type }),
    });
