...
(async () => {
  ...
  document.getElementById("dateDemande").min = today;
})();
  ...
  if (!services.length) {
    container.innerHTML = `
      <div class="text-center col-span-3 py-16">
        ...
        <p class="text-gray-400 italic">Aucun service trouvé.</p>
      </div>`;
    return;
  }
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            ${isSponsored ? `<span class="text-xs bg-[#FCE297] text-[#1A2B49] px-2 py-0.5 rounded-full font-fira"><iconify-icon icon="mdi:star-circle"></iconify-icon> Sponsorisé</span>` : ""}
            ${demandeActive ? `<span class="text-xs font-fira text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">En cours</span>` : ""}
          </div>
          <p class="text-sm text-gray-400">${esc(o.nom_entreprise || "Indépendant")}</p>
...
async function selectionnerPro(idOffre) {
  ...
  try {
    ...
    const planningHtml = planningPro.length
      ...
      : `<p class="text-gray-300 italic text-sm">Aucune disponibilité renseignée.</p>`;
      ...
      <div class="space-y-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-2 flex-wrap">
              ...
              ${isSponsored ? `<span class="text-xs bg-[#FCE297] text-[#1A2B49] px-2 py-1 rounded-full font-fira"><iconify-icon icon="mdi:star-circle"></iconify-icon> Sponsorisé</span>` : ""}
            </div>
            <p class="text-gray-400 text-sm">${esc(o.nom_entreprise || "Indépendant")}</p>
        ...
        ${
          o.bio
            ? `
        <div>
          <p class="font-fira uppercase text-xs tracking-widest text-gray-400 mb-2">À propos</p>
          <p class="text-gray-500 text-base leading-relaxed">${esc(o.bio)}</p>
        </div>`
            : ""
        }

        <div>
          <p class="font-fira uppercase text-xs tracking-widest text-gray-400 mb-3">
            <iconify-icon icon="mdi:clock-outline" class="mr-1"></iconify-icon>Disponibilités
          </p>
          ${planningHtml}
        </div>
        ...
        <div>
          <p class="font-fira uppercase text-xs tracking-widest text-gray-400 mb-3">
            <iconify-icon icon="mdi:star" class="mr-1"></iconify-icon>Derniers avis
          </p>
          ${avisHtml}
        </div>
...
function choisirPrestataire(idOffre, nomPro, prix) {
...
async function envoyerDemande() {
  const date = document.getElementById("dateDemande").value;
  if (!date) {
    showToast("Veuillez choisir une date.", "error");
    return;
  }
  if (new Date(date) < new Date()) {
    showToast("Choisissez une date dans le futur.", "error");
    return;
  }
  ...
  try {
    ...
    if (res.status === 409) {
      showToast("Vous avez déjà une demande pour cette date.", "error");
      fermerModalDemande();
      return;
    }
...
function renderDemandes(demandes) {
  ...
  container.innerHTML = demandes
    .map((d) => {
      ...
      const msgAnnulee =
        d.statut === "annulee"
          ? `<p class="text-gray-400 text-sm italic mt-2"><iconify-icon icon="mdi:information" class="mr-1"></iconify-icon>Demande expirée — le prestataire n'a pas répondu.</p>`
          : "";
    ...
    <div class="group bg-white p-8 rounded-[40px] border-2 ${isTermine ? "opacity-70 border-gray-200" : "border-transparent hover:border-[#7CABD3] hover:shadow-xl"} transition-all duration-500">
      ...
      <div class="space-y-1 mb-4">
        ...
        <p class="text-base text-gray-500"><iconify-icon icon="mdi:calendar" class="mr-1 text-[#7CABD3]"></iconify-icon>Date souhaitée : ${esc((d.date_souhaitee || "").split("T")[0])}</p>
