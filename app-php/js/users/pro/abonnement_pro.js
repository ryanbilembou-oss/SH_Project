  ...
  if (isSuccess) {
    ...
    while (tries < 5) {
      ...
      try {
        ...
        if (data && data.id_abonnement && data.statut === "actif") {
...
function afficherAbonnement(abo, estResilie) {
  ...
  document.getElementById("aboDebut").textContent = new Date(
    abo.date_debut,
  ).toLocaleDateString("fr-FR", opts);
  document.getElementById("aboFin").textContent = new Date(
    abo.date_fin,
  ).toLocaleDateString("fr-FR", opts);
  ...
  if (estResilie) {
    ...
    badge.className =
      "px-4 py-2 rounded-full font-fira text-sm uppercase bg-orange-100 text-orange-600";
    document.getElementById("msgResiliation").classList.remove("hidden");
    document.getElementById("aboFinResiliation").textContent = new Date(
      abo.date_fin,
    ).toLocaleDateString("fr-FR", opts);
    ...
    badge.className =
      "px-4 py-2 rounded-full font-fira text-sm uppercase bg-emerald-100 text-emerald-700";
