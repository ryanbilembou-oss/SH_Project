...
async function loadLitiges() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/admin/litiges/get_by_user?id_user=${userId}&role=pro`,
    );
...
function renderAccordeon(l) {
  ...
  const decisionBadge = l.decision
    ? l.decision === "pro"
      ...
      : `<span class="bg-red-100 text-red-600 text-xs px-3 py-1 rounded-full font-fira uppercase">Decision en faveur du senior</span>`
    : "";
    ...
    <div id="accordeon-${l.id_litige}" class="hidden px-6 pb-6">
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="bg-gray-50 px-4 py-3 rounded-[15px]">
          <p class="font-fira uppercase text-xs text-gray-400">Senior</p>
          <p class="font-fira text-[#1A2B49]">${esc(l.prenom_senior)} ${esc(l.nom_senior)}</p>
        </div>
        <div class="bg-gray-50 px-4 py-3 rounded-[15px]">
