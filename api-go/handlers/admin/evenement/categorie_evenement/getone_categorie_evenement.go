...
func GetOneCategorieEvenement(w http.ResponseWriter, r *http.Request) {
    ...
    err := database.DB.QueryRow(
        `SELECT id_categorie, nom_categorie FROM categorie_evenement WHERE id_categorie = $1`, id,
    ).Scan(&cat.IDCategorie, &cat.NomCategorie)

    if err != nil {
        log.Printf("❌ Erreur SQL getone catégorie : %v", err)
