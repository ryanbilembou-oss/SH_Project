...
func DeleteCategorieEvenement(w http.ResponseWriter, r *http.Request) {
	...
	err := database.DB.QueryRow(
		`SELECT id_categorie FROM categorie_evenement WHERE nom_categorie = 'Non classé' LIMIT 1`,
	).Scan(&nonClasseID)
	if err != nil {
	...
	_, err = database.DB.Exec(
		`UPDATE evenements SET id_categorie = $1 WHERE id_categorie = $2`,
	...
	_, err = database.DB.Exec(
		`DELETE FROM categorie_evenement WHERE id_categorie = $1`, req.IDCategorie,
	)
	if err != nil {
		log.Printf("Erreur SQL delete catégorie : %v", err)
		w.WriteHeader(http.StatusInternalServerError)
