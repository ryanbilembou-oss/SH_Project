...
func DeleteDocumentPro(w http.ResponseWriter, r *http.Request) {
	...
	result, err := database.DB.Exec(`DELETE FROM documents_pro WHERE id_doc = $1`, id)
	if err != nil {
		log.Printf("❌ DeleteDocumentPro - Erreur SQL: %v", err)
	...
	log.Printf("✅ DocumentPro %s supprimé", id)
